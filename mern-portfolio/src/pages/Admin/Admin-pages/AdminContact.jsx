...
function AdminContact() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const response = await axios.get('/api/admin/showcontact');
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`/api/admin/deletecontact/${id}`);
      ...
      <div className='flex w-full my-[10rem] mx-6 relative flex-wrap gap-5 overflow-hidden h-full'>
        {data.length > 0 ? (data.map((item, index) => (
            ...
            >
              ...
              <button
                className={`absolute p-2 right-5 top-5 ${hoveredIndex === index ? 'flex' : 'hidden'}`}
