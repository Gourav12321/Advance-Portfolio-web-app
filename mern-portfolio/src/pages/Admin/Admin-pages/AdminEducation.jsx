...
function AdminEducation() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!change.name || !change.degree || !change.grade || !change.startDate || !change.endDate || !change.message) {
      alert('All fields are required');
      return;
    }
    try {
      
      await axios.post('/api/admin/education', change);
      setChange({ name: '', degree: '', grade: '', startDate: '', endDate: '', message: '', achievements: '' });
    } catch (error) {
      console.error(error);
    }
  };
      ...
      <div className='flex flex-col justify-center items-center w-full relative z-10'>
        ...
        <div className='w-1/2'>
          <form className='w-full flex flex-col gap-3' onSubmit={handleSubmit}>
            ...
            />
            <button type='submit' className='z-10 button-29'>Submit</button>
          </form>
        </div>
      </div>
    </div>
