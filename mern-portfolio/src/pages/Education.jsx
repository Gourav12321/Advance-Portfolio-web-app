          ...
          {data.map((education, index) => (
            ...
            >
              <div className="w-full mx-5 mb-5 p-4 rounded-2xl border-[2px] border-[#FFC107] shadow-md shadow-indigo-300 transform transition-transform duration-300 group-hover:scale-105 group-hover:shadow-lg group-hover:shadow-indigo-500">
                ...
                <p className='text-[1rem] font-medium my-3'>{education.degree}</p>
                <p className='my-2'><span className='font-bold mr-2'>Grade:</span> {education.grade}</p>
                <p className='md:absolute right-9 top-4 bg-[#191923] text-slate-400 w-[11.3rem] px-4 py-2 rounded-full my-3 text-center'>{education.startDate} - {education.endDate}</p>
                  ...
                  <div>
                    ...
                    {showCertificate === index && (
                      <div className='mt-4 w-[5rem]'>
                        <a href={`${education.achievements}`} target="_blank" rel="noopener noreferrer">
                          <img 
